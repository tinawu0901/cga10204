...
public class CarScheduleFilter  implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (username == null) { // 沒登入過
			...
			List<PermissionVO> empPr = service.getByEmpNo(username.getEmp_no());
			int size = empPr.stream().filter(e -> e.getEmpf_no() == 6).collect(Collectors.toList()).size();
			if (size == 1) {
