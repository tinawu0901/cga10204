					...
					<div class="col-lg-5">
						...
						<div class="intro">
							...
							<div class="custom-breadcrumbs">
								<a href="<%= request.getContextPath()%>/index">首頁</a> <span class="mx-2">/</span> <strong>訂單列表</strong>
							</div>
						</div>
					...
					<tbody class="tbody" style="background-color: rgb(243, 250, 253);">
						<c:forEach var="member_order" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
							<tr>
								...
								<td><c:if test="${member_order.rcaro_pickuploc eq 'KH'}">高雄</c:if>
						<c:if test="${member_order.rcaro_pickuploc eq 'TPE'}">台北</c:if>
						<c:if test="${member_order.rcaro_pickuploc eq 'TC'}">台中</c:if></td>
								<c:choose>
								...
								<td><a
									...
									style='text-decoration: none; color: blue;'><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">
