		...
		<div class='divunder'>
			...
			<div class="divtext">
				...
				<div class='divr'>
					...
					<!-- 出價的地方 -->
					<form onsubmit="return ent();" class='formdo' id="form1" >
						<input class='inputdo' id="inputprice" placeholder="${errorMessages.bid_price}" name="bid_price" type="text">
						<input type="hidden" name="scar_no" value="${scarVO.scar_no}">
						<input class="btn btn-warning inputdobtn" id="offerprice" type="button" name="sub" value="我要出價" style="font-size: 15px; color: white; font-weight: 900; border-radius: 10px;">
						<input type="hidden" value="${scarVO.scar_startime}" name="sttime">
...
<!-- 			顯示最高價的人 -->
			<div style="text-align: center;">			
					<p style="color: red; font-weight: 900; font-size: 24px">目前出價最高者為: <span id="topmeb"><c:if test="${mebName != null}">${mebName}</c:if><c:if test="${mebName == null}">目前無人出價</c:if></span></p>
...
<!--  	</script>  -->
	...
	function ent(){
		...
		$.ajax({
            type: "POST", 
            url: "/CGA102G4/scar/offerPrice", 
            data: new FormData( document.getElementById('form1') ), 
            contentType: false,//must, tell jQuery not to process the data 
            processData: false, 
            dataType: 'text', 
            success: function (data) {
            	if(data == '出價須1000為單位且高於目前價格'){
            		swal('出價最少1000一加且高於目前價格','請重新出價','warning');
            	}else if(data == '出價不能為空'){
            		swal('出價不能為空','請重新出價','warning');
            	}else if(data == '請輸入正確數字格式'){
            		swal('請輸入正確數字格式','請重新出價','warning');
            	}else if(data == '最高價是你了'){
            		swal('最高價是你了','請重新出價','warning');
            	}else if(data == '資料發生錯誤'){
            		swal('資料發生錯誤','請重新出價','warning');
            	}else if(data == '出價成功'){
            		swal('出價成功','','warning');
            	}
            }
    ...
    $(document).on('click',"#offerprice",function (e) { 
					$.ajax({
			            type: "POST", 
			            url: "/CGA102G4/scar/offerPrice", 
	...
	<script>
		let stime = $('#offerprice').next().val();
		if(new Date($.now()) < new Date(stime)){
			document.querySelector('#offerprice').setAttribute('disabled','true');
		}
	</script>
