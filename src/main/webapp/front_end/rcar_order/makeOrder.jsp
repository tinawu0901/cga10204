...
<%
// 	RcarOrderVO orderVO = (RcarOrderVO)request.getSession().getAttribute("orderVO");
...
<link   rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/jquery.datetimepicker.css" />
		...
		<%-- 自己的js --%>
		<script src="${pageContext.request.contextPath}/front_end/rcar_order/js/validation.js"></script>
		<script>
			...
			$(function(){
				 ...
				 $('#order_return_car_time').datetimepicker({
				  ...
				  onShow:function(){
					...
					nextDate = new Date($('#order_car_time').val())
                	nextDate.setDate(nextDate.getDate()+1);
                
	                this.setOptions({
	                    minDate: $('#order_car_time').val()?nextDate:false
			...
			<div class="customer_info">
				...
				<div class="div_start_store left_side" style="margin-left:10px;">
					...
					<select name="start_store" id="pickStore">
						...
						<c:forEach var="store" items="${stores}">
							<option value = "${store.st_no}" class = "${store.st_no}" ${store.st_no.equals(orderVO.rcaro_pickuploc) ? 'selected':''}>${store.st_name}</option>
						</c:forEach>
					</select>
				...
				<div class="div_return_store right_side">
					...
					<select name="return_store" id="backStore"  >
						<option value="unpicked">請選擇</option>
						<c:forEach var="store" items="${stores}">
							<option value = "${store.st_no}" class = "${store.st_no}" ${store.st_no == orderVO.rcaro_returnloc? 'selected':''}>${store.st_name}</option>
						</c:forEach>
					</select>
				...
				<div class="div_order_car_time left_side">
					...
					<input type="text" id = "order_car_time" name="order_car_time" value = "${orderVO.rcaro_ppicktime }">
				...
				<div class="div_order_return_time right_side">
					...
					<input type="text" id="order_return_car_time" name="order_return_car_time" value = "${orderVO.rcaro_pprettime }"> 
				...
				<div class="div_car_level left_side">
					...
					<select name="car_level" id="car_level_id">
					<option value="unpicked">請選擇</option>
						
						<c:forEach var="level" items="${carLevels}">
						<option value="${level.getLevel_no()}" class = "${level.getLevel_no()}" ${orderVO.level_no.equals(level.level_no)? 'selected':''}>${level.level_name}</option>
						</c:forEach>
				...
				<div class="div_car_model right_side">
					...
					<c:forEach var="model" items="${carModels}">
						<option value="${model.model_no}" id="${model.model_no}_id" ${model.model_no.equals(orderVO.model_no)? 'selected' : ''}>${model.model_no}</option>
						</c:forEach>
					</select>
